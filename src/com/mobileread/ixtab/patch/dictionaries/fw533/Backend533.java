package com.mobileread.ixtab.patch.dictionaries.fw533;
...
public class Backend533 implements ActionListener, PropertyChangeListener, Backend {
	...
	private static Backend533 INSTANCE;
	...
	private static void modifyLinkPanel(f linkPanel) {
	...
	private void gotoDict(final Integer newPosition) {
		if (newPosition != null) {
			SwingUtilities.invokeLater(new Runnable() {
				...
				public void run() {
					dictCard.zo(newPosition.intValue());
					...
					if (dialog instanceof KDialog) {
						((KDialog)dialog).setTitle(dictCard.So(newPosition.intValue()));
					}
					dialog.validate();
					dialog.repaint();
				}
