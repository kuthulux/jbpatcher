package com.mobileread.ixtab.patch.dictionaries.fw531;
...
public class Backend531 implements ActionListener, PropertyChangeListener, Backend {
	...
	private static Backend531 INSTANCE;
	...
	private void gotoDict(final Integer newPosition) {
		if (newPosition != null) {
			SwingUtilities.invokeLater(new Runnable() {
				...
				public void run() {
					dictCard.cO(newPosition.intValue());
					...
					if (dialog instanceof KDialog) {
						((KDialog)dialog).setTitle(dictCard.iO(newPosition.intValue()));
					}
					dialog.validate();
					dialog.repaint();
				}
